...
/*
    private boolean isDependsCompleted() {
        ...
        for (Enumeration en = v.elements(); en.hasMoreElements();) {
            ...
            if (t.getStatus() != ITask.COMPLETED)
                check = false;
        }
        return check;
    }
...
*/
	...
	// getsubtask and hassubtasks had duplicate code between the two with only one difference
	// so i made the duplicate code its own method and let the original methodes return what they needed
	/* 
	...
	public boolean hasSubTasks(String id) {
		if (checktask(id) != null)
				return true;
		return false;
	}

	public ITask checktask(String id) {
	    Elements subTasks = _element.getChildElements("task");
